...
public actual open class LockFreeLinkedListNode {
    private val _next = atomic<Any?>(null) // Node | Removed | OpDescriptor
    private val _prev = atomic<Any?>(null) // Node | Removed
    ...
    private fun expectNextThis(): Any? = if (_next.value == null) null else this
    private fun expectPrevThis(): Any? = if (_prev.value == null) null else this

    private fun patchNextNull() {
        if (_next.value == null) _next.compareAndSet(null, this) // patch _next from null to this
    }
    ...
    public val next: Any get() {
        _next.loop { nextValue ->
            val next = nextValue ?: this
    ...
    public val prev: Any get() {
        _prev.loop { prevValue ->
            val prev = prevValue ?: this
    ...
    public open class AddLastDesc<T : Node> constructor(
        ...
        init {
            ...
            check(node._next.value == null && node._prev.value == null)
        }

        final override fun takeAffectedNode(op: OpDescriptor): Node {
            while (true) {
                val prev = (queue._prev.value as Node?) ?: queue // this sentinel node is never removed
                val next = prev._next.value ?: prev
        ...
        override fun updatedNext(affected: Node, next: Node): Any {
            ...
            node._prev.compareAndSet(node.expectPrevThis(), affected)
            node._next.compareAndSet(node.expectNextThis(), queue)
            return node
        }
    ...
    public abstract class AbstractAtomicDesc : AtomicDesc() {
        ...
        final override fun prepare(op: AtomicOp<*>): Any? {
            while (true) { // lock free loop on next
                ...
                val nextValue = affected._next.value
                val next = nextValue ?: affected
                ...
                if (affected._next.compareAndSet(nextValue, prepareOp)) {
    ...
    private fun finishAdd(next: Node) {
        next._prev.loop { nextPrevValue ->
            val nextPrev = nextPrevValue ?: next
            if (nextPrev is Removed || this.next !== next) return // next was removed, remover fixes up links
            if (next._prev.compareAndSet(nextPrevValue, this)) {
        ...
        while (true) {
            ...
            if (prevNext is Removed) {
                if (last != null) {
                    ...
                    prev = (prev._prev.value ?: prev).unwrap()
                }
                continue
            }
            if (prevNext !== this) {
    ...
    private fun correctPrev(_prev: Node, op: OpDescriptor?): Node? {
        ...
        while (true) {
            // move the the left until first non-removed node
            val prevNextValue = prev._next.value
            val prevNext = prevNextValue ?: prev
            ...
            val oldPrevValue = this._prev.value
            val oldPrev = oldPrevValue ?: this
            ...
            if (this._prev.compareAndSet(oldPrevValue, prev)) {
