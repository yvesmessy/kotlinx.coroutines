...
open class SequencePlaysScrabbleAsFlow : ShakespearePlaysScrabble() {

    @Benchmark
    public override fun play(): List<Map.Entry<Int, List<String>>> {
        val score2: suspend (String) -> Int = { word: String ->
            buildHistogram(word)
                .map { it.letterScore() }
                .sum()
        }

        val bonusForDoubleLetter: suspend (String) -> Int = { word: String ->
            toBeMaxed(word)
    ...
    private fun toBeMaxed(word: String) = concat(word.asSequence(startIndex = 3), word.asSequence(endIndex = 3))

    private suspend fun checkBlanks(word: String) = numBlanks(word) <= 2L
    ...
    private suspend fun buildHistogram(word: String): HashMap<Int, MutableLong> {
